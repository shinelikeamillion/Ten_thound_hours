...
public class DotsView extends View{
    ...
    private void drawInnerDotsFrame(Canvas canvas) {
        for (int i = 0; i <DOTS_COUNT; i++) {
            ...
            canvas.drawCircle(cX, cY, currentDotSize2, dotPaints[(i + 1) % dotPaints.length]);
        }
    }
    ...
    private void updateOuterDotsPosition() {
        if (currentProgress < 0.3f) {
            ...
            this.currentRadius1 = (float) Utils.mapValueFromRangeToRange(
                    currentProgress, 0.3f, 1f, 0.8f * maxOuterDotsRadius,
                    maxOuterDotsRadius);
        }
    ...
    private void updateDotsPaints () {
        if (currentProgress < 0.5f) {
            ...
            dotPaints[0].setColor((Integer) argbEvaluator.evaluate(progress, COLOR_2, COLOR_3));
            dotPaints[1].setColor((Integer) argbEvaluator.evaluate(progress, COLOR_3, COLOR_4));
            dotPaints[2].setColor((Integer) argbEvaluator.evaluate(progress, COLOR_4, COLOR_1));
            dotPaints[3].setColor((Integer) argbEvaluator.evaluate(progress, COLOR_1,COLOR_2));
        }
    }
